...
const LoadingScreen=()=> { const {colors}=useAppContext(); return <div className="min-h-screen flex flex-col items-center justify-center p-4" style={{background:colors.gradient}}><div className="text-center"><h1 className="text-5xl sm:text-6xl md:text-8xl font-bold tracking-wider relative" style={{color:colors.text}}>SYNAPSE<div className="absolute right-[-1.5rem] md:right-[-2rem] top-1/2 -translate-y-1/2 w-3 h-3 md:w-5 md:h-5"><div style={{backgroundColor:colors.accent}} className="absolute w-full h-full rounded-full animate-ping"></div><div style={{backgroundColor:colors.accent}} className="absolute w-full h-full rounded-full"></div></div></h1><p className="text-md md:text-lg opacity-75 mt-4">Loading your personalized learning experience...</p></div></div>; };
...
const ContentManager = () => {
    ...
    useEffect(() => {
        ...
        if (subject && (!selectedModule || !subject.modules.find(m => m.title === selectedModule))) {
